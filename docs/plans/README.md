...
|-------|------|----------|------------------|------|
| 1 | 项目地基 | — | `docker run` 起服务,前端能打开,/healthz 通,CI 绿 | **本期已写**:`2026-05-09-phase-1-foundation.md` |
| 2 | 视觉链路 | M1 + M2 + M3 | 上传图片 → 安全检查 → 显示可点击热区 | 待写(完成 Phase 1 后) |
| 3 | 对话骨架 | M5 + M6(文字部分) | 点击热区 → 文字 WebSocket 对话(无语音) | 待写 |
| 4 | 语音闭环 | M4 + M6(语音流式) | STT/TTS/lip-sync 全通,真实语音对话 | 待写 |
| 5 | 教学+存储+收尾 | M7 + M8 + M9 + M10 | 深度教学反馈、IndexedDB、水平自选、对话总结卡 | 待写 |
